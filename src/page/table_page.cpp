...
bool TablePage::InsertTuple(Row &row, Schema *schema, Transaction *txn,
  ...
  serialized_size = row.GetSerializedSize(schema);
...
int TablePage::UpdateTuple(const Row &new_row, Row *old_row, Schema *schema,
  ...
  // If the slot number is invalid, abort. 1.记录位置错误
  if (slot_num >= GetTupleCount()) {
    return false;
  }
  uint32_t tuple_size = GetTupleSize(slot_num);
  // If the tuple is deleted, abort. 2.记录标记删除 3.记录已经删除
  if (IsDeleted(tuple_size)) {
    return false;
  }
  // If there is not enough space to update, we need to update via delete followed by an insert (not enough space).
  // 4.当前页面空间不足，返回table_heap进行删除、插入
