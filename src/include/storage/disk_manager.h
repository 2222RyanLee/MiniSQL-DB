...
public:
  ...
  static constexpr size_t BITMAP_SIZE = BitmapPage<PAGE_SIZE>::GetMaxSupportedSize();//位图能存储的最大页数

private:
  /**
   * Helper function to get disk file size
   */
  int GetFileSize(const std::string &file_name);//传入文件名字，返回文件大小
...
private:
  ...
  std::fstream db_io_;//操作文件
  std::string file_name_;//文件名字
  // with multiple buffer pool instances, need to protect file access
  std::recursive_mutex db_io_latch_;
  bool closed{false};//判断文件是否关闭
  char meta_data_[PAGE_SIZE];//需要转化为MetaPage对象
};
