...
private:
  static_assert(sizeof(page_id_t) == 4);//页号4B
  static constexpr uint64_t DELETE_MASK = (1U << (8 * sizeof(uint32_t) - 1));//
  static constexpr size_t SIZE_TABLE_PAGE_HEADER = 24;//header 24B
  static constexpr size_t SIZE_TUPLE = 8;//记录的
  static constexpr size_t OFFSET_PREV_PAGE_ID = 8;//prevent page的偏移起始
  static constexpr size_t OFFSET_NEXT_PAGE_ID = 12;//next page 的偏移起始
  static constexpr size_t OFFSET_FREE_SPACE = 16;//空白区起始地址
  static constexpr size_t OFFSET_TUPLE_COUNT = 20;//20-23 总记录条数
  static constexpr size_t OFFSET_TUPLE_OFFSET = 24;//24-27 首记录偏移量
  static constexpr size_t OFFSET_TUPLE_SIZE = 28;//28-31 首记录大小

public:
  static constexpr size_t SIZE_MAX_ROW = PAGE_SIZE - SIZE_TABLE_PAGE_HEADER - SIZE_TUPLE;
  //最大记录数=页面字节数-24B_header-首记录偏移量-首记录大小
};
