...
public:
  ...
  std::string BackValue(){
    if(this->type_id_==kTypeInt){
      return std::to_string(this->value_.integer_);
    }
    else if(this->type_id_==TypeId::kTypeFloat) return std::to_string(this->value_.float_);
    else if(this->type_id_==TypeId::kTypeChar) return this->value_.chars_;
    else return "";
