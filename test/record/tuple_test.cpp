...
Column c[] = {
        Column(column_name, TypeId::kTypeInt, 1, true, true),
        Column(column_name, TypeId::kTypeChar,3,1,true,true)
};
TEST(Test,ColumnTest_Schematest){
char buf[100]; //序列化
int inset = 0;
char *q = buf;
for (int i = 0;i<2;i++){
  u_int32_t temp = c[i].SerializeTo(q);
  q+=temp;
  inset+=temp;
}
MemHeap *heap_ = new SimpleMemHeap();
int offset = 0;
Column *d = nullptr;
char *t = new char[100];
memcpy(t,buf,sizeof(buf));
for (int i = 0;i<2;i++){//反序列化
  offset+= Column::DeserializeFrom(t+offset,d,heap_);
}
EXPECT_EQ(inset,offset);    //判断序列化和反序列化偏移位数是否相等
vector<Column*> v={&c[0],&c[1]};
//memcpy(&v[0], c, sizeof(c));
char uf[100]; //序列化
Schema s(v);   //将column数组初始化为一个schema
char *m = uf;
u_int32_t ta = s.SerializeTo(m);
Schema *df = nullptr;
u_int32_t sa = Schema::DeserializeFrom(m,df,heap_);//反序列化
EXPECT_EQ(ta,sa);   //判断序列化和反序列化偏移位数是否相等
};
